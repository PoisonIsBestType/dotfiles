...
-- https://github.com/streetturtle/widget.todo/tree/master/calendar-widget
...
local function worker(user_args)
    ...
    local calendar_themes = {
        nord = {
            ...
            focus_date_bg = beautiful.accent_nrm,
            focus_date_fg = beautiful.bg_focus,
            weekend_day_bg = beautiful.bg_urgent,
            weekday_fg = beautiful.accent_nrm,
            header_fg = beautiful.accent_nrm,
            border = beautiful.accent_nrm
        },
        naughty = {
    ...
    local placement = args.placement or 'top_left'
    ...
    local function decorate_cell(widget, flag, date)
        ...
        local ret = wibox.widget {
            ...
            shape = props.shape,
            shape_border_color = props.border_color or beautiful.fg_normal,
            shape_border_width = props.border_width or 0.5,
    ...
    function calendar_widget.toggle()
        ...
        if popup.visible then
            ...
            if placement == 'top' then
                awful.placement.top(popup, { margins = { top = 52 }, parent = awful.screen.focused() })
            elseif placement == 'top_right' then
                awful.placement.top_right(popup, { margins = { top = 52, right = 10}, parent = awful.screen.focused() })
            elseif placement == 'top_left' then
                awful.placement.top_left(popup, { margins = { top = 52, left = 30}, parent = awful.screen.focused() })
            elseif placement == 'bottom_right' then
                awful.placement.bottom_right(popup, { margins = { bottom = 52, right = 10},
                    parent = awful.screen.focused() })
            elseif placement == 'bottom_left' then
                awful.placement.bottom_left(popup, { margins = { bottom = 52, left = 10},
                    parent = awful.screen.focused() })
            else
                awful.placement.top(popup, { margins = { top = 52 }, parent = awful.screen.focused() })
            end
